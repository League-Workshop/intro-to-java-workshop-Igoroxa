...
public class DragonFight {
	...
	public static void main(String[] args) {
		...
		while (playerHealth > 0 && dragonHealth > 0) { // this line of code keeps the battle going until someone's
			...
			// or a kick
			String input = JOptionPane.showInputDialog("What do you want to attack with? Yell or Kick.");
			// 9. If they typed in "yell":
			if (input.equalsIgnoreCase("yell")) {
				// -- Find a random number between 0 and 10 and store it in dragonDamage
				Random playerdamage = new Random();
				ddamage = playerdamage.nextInt(15);
			...
			if (input.equalsIgnoreCase("kick")) {
				...
				Random playerdamage2 = new Random();
				ddamage = playerdamage2.nextInt(15);
				// -- Subtract that number from the dragon's health variable
				dragonHealth = dragonHealth - ddamage;
			}
			// 11. Find a random number between 0 and 35 and store it in playerDamage
			Random dragondamage = new Random();
			pdamage = dragondamage.nextInt(15);
			// 12. Subtract this number from the player's health
			playerHealth = playerHealth - pdamage;
			// 13. If the user's health is less than or equal to 0
			if (playerHealth <= 0) {
				// -- Tell the user that they lost
				JOptionPane.showMessageDialog(null, "You lost bot");
			}
